...
function createLLMClient(baseURL, apiKey) {
  return new OpenAI({ baseURL, apiKey, timeout: 5 * 60 * 1000 });
}
...
// Lazy fallback client — only created when fallbackProvider is configured in user-config.json
let _fallbackClient = null;
function getFallbackClient() {
  const fb = config.llm.fallback;
  if (!_fallbackClient && fb?.baseUrl && fb?.apiKey) {
    _fallbackClient = createLLMClient(fb.baseUrl, fb.apiKey);
    log("agent", `Fallback client initialized → ${fb.baseUrl}`);
  }
...
export async function agentLoop(goal, maxSteps = config.llm.maxSteps, sessionHistory = [], agentType = "GENERAL", model = null, maxOutputTokens = null, options = {}) {
  ...
  const systemPrompt = await buildSystemPrompt(agentType, portfolio, positions, stateSummary, lessons, perfSummary, weightsSummary, decisionSummary);
  ...
  for (let step = 0; step < maxSteps; step++) {
    ...
    try {
      ...
      // Determine fallback model: per-role override > generic fallback model > primary model
      const fb = config.llm.fallback;
      const fallbackModel = fb
        ? (agentType === "SCREENER" ? fb.screeningModel
          : agentType === "MANAGER" ? fb.managementModel
          : fb.generalModel)
          || fb.model
          || activeModel
        : activeModel;
      ...
      let activeClient = client; // start with primary provider
      ...
      for (let attempt = 0; attempt < 3; attempt++) {
        ...
        try {
          ...
          // Check if this is a transient HTTP error (thrown by SDK as exception)
          const httpStatus = error.status || error.statusCode || error.code;
          if (httpStatus === 502 || httpStatus === 503 || httpStatus === 504 || httpStatus === 529 || httpStatus === 429) {
            // Treat as transient — fall through to the shared fallback logic below
        ...
        // --- Shared transient error + fallback logic (both thrown exceptions and response-level errors) ---
        if (response.choices?.length) break;
        const errCode = response.error?.code;
        const nullChoices = !response.choices || response.choices.length === 0;
        if (errCode === 502 || errCode === 503 || errCode === 504 || errCode === 529 || errCode === 429 || nullChoices) {
          const wait = (attempt + 1) * 5000;
          const reason = nullChoices ? "empty response" : `HTTP ${errCode}`;
          if (attempt === 1 && activeClient === client) {
            // Switch to fallback provider + model
            const fbClient = getFallbackClient();
            if (fbClient) {
              activeClient = fbClient;
              usedModel = fallbackModel;
              log("agent", `${reason} — switching to fallback provider (${config.llm.fallback.baseUrl}) model ${fallbackModel}`);
            } else {
              usedModel = fallbackModel;
              log("agent", `${reason} — switching to fallback model ${fallbackModel} (no fallback provider)`);
            }
          } else {
            log("agent", `${reason}, retrying in ${wait / 1000}s (attempt ${attempt + 1}/3)`);
