...
const configuredMinBinsBelow =
  numericConfig(u.minBinsBelow) ?? MIN_SAFE_BINS_BELOW;
const configuredMaxBinsBelow =
  numericConfig(u.maxBinsBelow) ??
  (legacyBinsBelow != null
    ? Math.max(legacyBinsBelow, configuredMinBinsBelow)
    : 69);
const configuredDefaultBinsBelow =
  numericConfig(u.defaultBinsBelow) ??
  legacyBinsBelow ??
  configuredMaxBinsBelow;
const strategyMinBinsBelow = Math.max(
  MIN_SAFE_BINS_BELOW,
  Math.round(configuredMinBinsBelow),
);
const strategyMaxBinsBelow = Math.max(
  strategyMinBinsBelow,
  Math.round(configuredMaxBinsBelow),
);
...
if (u.agentMeridianApiUrl)
  process.env.AGENT_MERIDIAN_API_URL ||= u.agentMeridianApiUrl;
...
export const config = {
  ...
  screening: {
    ...
    maxIndicatorRejections: u.maxIndicatorRejections ?? 5, // max indicator rejections in 24h before auto-skip
  },

  // ─── GMGN Screening Pipeline ─────────────
  gmgn: {
    apiKey: nonEmptyString(
      gmgnUserConfig.apiKey,
      u.gmgnApiKey,
      process.env.GMGN_API_KEY,
    ),
    baseUrl: nonEmptyString(
      gmgnUserConfig.baseUrl,
      u.gmgnBaseUrl,
    ...
    klineLookbackMinutes: gmgnValue(
      "klineLookbackMinutes",
      "gmgnKlineLookbackMinutes",
      60,
    ),
    filters: gmgnArray("filters", "gmgnFilters", [
      "renounced",
      "frozen",
      "not_wash_trading",
    ]),
    platforms: gmgnArray("platforms", "gmgnPlatforms", [
      "Pump.fun",
    ...
    maxTokenAgeHours: gmgnValue(
      "maxTokenAgeHours",
      "gmgnMaxTokenAgeHours",
      24 * 7,
    ),
    minSmartDegenCount: gmgnValue(
      "minSmartDegenCount",
    ...
    maxTop10HolderRate: gmgnValue(
      "maxTop10HolderRate",
      "gmgnMaxTop10HolderRate",
      0.5,
    ),
    maxBundlerRate: gmgnValue("maxBundlerRate", "gmgnMaxBundlerRate", 0.5),
    maxRatTraderRate: gmgnValue(
      "maxRatTraderRate",
      "gmgnMaxRatTraderRate",
      0.2,
    ),
    maxFreshWalletRate: gmgnValue(
      "maxFreshWalletRate",
      "gmgnMaxFreshWalletRate",
      0.2,
    ),
    maxDevTeamHoldRate: gmgnValue(
      "maxDevTeamHoldRate",
      "gmgnMaxDevTeamHoldRate",
      0.02,
    ),
    preferredKolMinHoldPct: gmgnValue(
      "preferredKolMinHoldPct",
      "gmgnPreferredKolMinHoldPct",
      1,
    ),
    dumpKolMinHoldPct: gmgnValue(
      "dumpKolMinHoldPct",
      "gmgnDumpKolMinHoldPct",
      0.5,
    ),
  ...
  management: {
    ...
    autoSwapMinUsd: u.autoSwapMinUsd ?? 0.1,
    ...
    repeatDeployCooldownMinFeeEarnedPct:
      u.repeatDeployCooldownMinFeeEarnedPct ??
      u.repeatDeployCooldownMinFeeYieldPct ??
      0,
    ...
    // Fee-protected claim — claim fees only when floating PnL covers them
    feeProtectedClaimEnabled: u.feeProtectedClaimEnabled ?? false,
    feeProtectedClaimMinUsd: u.feeProtectedClaimMinUsd ?? 0.4,
    feeProtectedClaimPnlRatio: u.feeProtectedClaimPnlRatio ?? 0.5,
    // Fee-decay exit — close if position is underwater and fees don't cover the loss
    feeDecayExitEnabled: u.feeDecayExitEnabled ?? false,
    feeDecayExitAgeMinutes: u.feeDecayExitAgeMinutes ?? 180,
    feeDecayExitPnlPct: u.feeDecayExitPnlPct ?? -5,
    feeDecayExitCoverageRatio: u.feeDecayExitCoverageRatio ?? 0.5,
    // Profit-decay exit — close if profit peaked then dropped by threshold
    profitDecayExitEnabled: u.profitDecayExitEnabled ?? false,
    ...
    deadFlowExitUnclaimedUsd: u.deadFlowExitUnclaimedUsd ?? 0.1,
  ...
  llm: {
    ...
    // Fallback provider — different provider/API key/model when primary fails
    fallback: u.fallbackProvider
      ? {
          baseUrl: u.fallbackProvider.baseUrl,
          apiKey: u.fallbackProvider.apiKey,
          model: u.fallbackProvider.model ?? null,
          screeningModel: u.fallbackProvider.screeningModel ?? null,
          managementModel: u.fallbackProvider.managementModel ?? null,
          generalModel: u.fallbackProvider.generalModel ?? null,
        }
      : null,
  },
  ...
  indicators: {
    ...
    // Indicator-based early exit (poller)
    exitIndicatorEnabled: indicatorUserConfig.exitIndicatorEnabled ?? false,
    exitIndicatorPreset: indicatorUserConfig.exitIndicatorPreset ?? "rsi_plus_supertrend",
    exitIndicatorMinPnlPct: indicatorUserConfig.exitIndicatorMinPnlPct ?? -2,
    exitIndicatorConfirmations: indicatorUserConfig.exitIndicatorConfirmations ?? 3,
    exitIndicatorCooldownMinutes: indicatorUserConfig.exitIndicatorCooldownMinutes ?? 5,
    exitIndicatorCheckIntervalSeconds: indicatorUserConfig.exitIndicatorCheckIntervalSeconds ?? 120,
  },
};
...
export function reloadScreeningThresholds() {
  try {
    ...
    if (fresh.minTokenFeesSol != null)
      s.minTokenFeesSol = fresh.minTokenFeesSol;
    if (fresh.maxTop10Pct != null) s.maxTop10Pct = fresh.maxTop10Pct;
    if (fresh.useDiscordSignals !== undefined)
      s.useDiscordSignals = fresh.useDiscordSignals;
    if (fresh.discordSignalMode != null)
      s.discordSignalMode = fresh.discordSignalMode;
    ...
    if (fresh.minQuoteOrganic != null)
      s.minQuoteOrganic = fresh.minQuoteOrganic;
    ...
    if (fresh.avoidPvpSymbols !== undefined)
      s.avoidPvpSymbols = fresh.avoidPvpSymbols;
    if (fresh.blockPvpSymbols !== undefined)
      s.blockPvpSymbols = fresh.blockPvpSymbols;
    if (fresh.maxBotHoldersPct != null)
      s.maxBotHoldersPct = fresh.maxBotHoldersPct;
    if (fresh.allowedLaunchpads !== undefined)
      s.allowedLaunchpads = fresh.allowedLaunchpads;
    if (fresh.blockedLaunchpads !== undefined)
      s.blockedLaunchpads = fresh.blockedLaunchpads;
    const minBinsBelow =
      numericConfig(fresh.minBinsBelow) ?? config.strategy.minBinsBelow;
    const maxBinsBelow =
    ...
    config.strategy.minBinsBelow = Math.max(
      MIN_SAFE_BINS_BELOW,
      Math.round(minBinsBelow),
    );
    config.strategy.maxBinsBelow = Math.max(
      config.strategy.minBinsBelow,
      Math.round(maxBinsBelow),
    );
