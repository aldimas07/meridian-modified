...
export async function runManagementCycle({ silent = false } = {}) {
  ...
  try {
    ...
    for (const p of positionData) {
      ...
      const pnlUsd = p.pnl_true_usd ?? p.pnl_usd ?? 0;
      const feeProtectedMinUsd =
        config.management.feeProtectedClaimMinUsd ?? 0.4;
      const feeProtectedRatio =
        config.management.feeProtectedClaimPnlRatio ?? 0.5;

      // Fee-protected claim: lock in earned fees only when floating PnL is also
      // strong enough relative to the fees being claimed.
      if (
        config.management.feeProtectedClaimEnabled &&
        claimableUsd >= feeProtectedMinUsd &&
        pnlUsd > 0 &&
        pnlUsd >= claimableUsd * feeProtectedRatio
      ) {
        actionMap.set(p.position, {
          action: "CLAIM",
          rule: "fee_protected",
          reason: `PnL $${pnlUsd.toFixed(2)} >= ${(feeProtectedRatio * 100).toFixed(0)}% of claimable fees $${claimableUsd.toFixed(2)}`,
        });
        continue;
      }
      // Claim rule
...
function getDeterministicCloseRule(position, managementConfig, poolData = null) {
  ...
  if (
    !pnlSuspect &&
    managementConfig.feeDecayExitEnabled &&
    ageMinutes >= (managementConfig.feeDecayExitAgeMinutes ?? 180) &&
    ...
    return {
      action: "CLOSE",
      rule: 6,
      reason: `fee-rich decay: PnL ${pnlPct.toFixed(2)}%, fees $${feesUsd.toFixed(2)} cannot cover loss $${Math.abs(pnlUsd).toFixed(2)}`,
    };
  }

  if (
    !pnlSuspect &&
    managementConfig.profitDecayExitEnabled &&
    ageMinutes >= (managementConfig.profitDecayExitAgeMinutes ?? 180) &&
    peakPnlPct >= (managementConfig.profitDecayExitPeakPct ?? 2) &&
    pnlPct <= peakPnlPct - (managementConfig.profitDecayExitDropPct ?? 1.5)
  ) {
    return {
      action: "CLOSE",
      rule: 7,
