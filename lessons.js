...
// Recalc trigger interval — read from config.darwin.recalcEvery at call time (default 5)
const DEFAULT_RECALC_EVERY = 5;
...
export async function recordPerformance(perf) {
  ...
  // Evolve thresholds every N closed positions (from config)
  const { config, reloadScreeningThresholds } = await import("./config.js");
  const recalcEvery = config.darwin?.recalcEvery ?? DEFAULT_RECALC_EVERY;
  if (data.performance.length > 0 && data.performance.length % recalcEvery === 0) {
...
export function evolveThresholds(perfData, config) {
  if (!perfData || perfData.length < (config?.darwin?.recalcEvery ?? DEFAULT_RECALC_EVERY)) return null;
