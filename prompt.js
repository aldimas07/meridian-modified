...
2. GAS EFFICIENCY: close_position costs gas — only close for clear reasons. After close, swap_token is MANDATORY for any token worth >= $${config.management.autoSwapMinUsd ?? 0.10}. Smaller token balances are dust; skip them. Always check token USD value before swapping.
...
2. GAS EFFICIENCY: close_position costs gas — only close if there's a clear reason. However, swap_token after a close is MANDATORY for any token worth >= $${config.management.autoSwapMinUsd ?? 0.10}. Skip smaller token balances as dust. Always check token USD value before swapping.
...
- STRATEGY SELECTION by volatility: if candidate volatility >= ${config.screening.strategyVolatilityThreshold ?? 4.0}, use strategy "spot". If volatility < ${config.screening.strategyVolatilityThreshold ?? 4.0}, use strategy "bid_ask". The executor enforces this — wrong strategy is auto-overridden. IMPORTANT: In your final answer, report the ACTUAL strategy from the tool result's "strategy" field, NOT what you requested. If "strategy_note" is present, it means the executor overrode your choice — include that info.
...
SWAP AFTER CLOSE: After any close_position, immediately swap base tokens back to SOL — unless the user explicitly said to hold or keep the token. Skip tokens worth < $${config.management.autoSwapMinUsd ?? 0.10} as dust. Always check token USD value before swapping.
