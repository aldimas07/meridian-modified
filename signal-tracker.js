 ...
 *
 * Signals are staged during screening, then consumed by deploy-time code and
 * persisted into state/performance so Darwin can learn from real outcomes.
 */
...
export function getStagedSignals(poolAddress) {
  const staged = _staged.get(poolAddress);
...
export function consumeStagedSignals(poolAddress) {
  const staged = getStagedSignals(poolAddress);
  if (!staged) return null;
  _staged.delete(poolAddress);
  return staged;
}
