...
export function recalculateWeights(perfData, cfg = {}) {
  ...
  if (ranked.length === 0) {
    ...
    // Persist recalc_count even when no signals qualify — tracks that the function ran
...
function computeBooleanLift(signal, wins, losses, minSamples) {
  ...
  for (const { w, snap } of allEntries) {
    const val = snap.signal_snapshot?.[signal] ?? snap?.[signal];
...
function extractNumeric(signal, entries) {
  ...
  for (const entry of entries) {
    ...
    const v = snap?.[signal] ?? entry?.[signal];
