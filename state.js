...
/**
 * Purge closed positions older than maxAgeMs (default 7 days).
 * Also caps closed positions to maxKept (50) most recent entries.
 * Keeps state.json lean and prevents memory bloat.
 * Returns the number of purged positions.
 */
export function purgeClosedPositions(maxAgeMs = 7 * 24 * 60 * 60 * 1000, maxKept = 50) {
  const state = load();
  const now = Date.now();
  let purged = 0;
  ...
  for (const posId in state.positions) {
    ...
    if (now - closedAt > maxAgeMs) {
      delete state.positions[posId];
      purged++;
    }
  }

  // Pass 2: cap closed positions to maxKept most recent
  const closedEntries = Object.entries(state.positions)
    .filter(([, p]) => p.closed && p.closed_at)
    .sort((a, b) => new Date(b[1].closed_at) - new Date(a[1].closed_at));

  if (closedEntries.length > maxKept) {
    for (const [posId] of closedEntries.slice(maxKept)) {
      delete state.positions[posId];
      purged++;
    }
  }
  ...
  if (purged > 0) {
    save(state);
    log("state", `Purged ${purged} closed positions (age cap: ${Math.round(maxAgeMs / 86400000)}d, max kept: ${maxKept})`);
  }
