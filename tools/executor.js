      ...
      if (name === "swap_token" && result.tx) {
        ...
        if (!args.skip_swap && result.base_mint) {
          try {
            ...
            const autoSwapMinUsd = Number(config.management.autoSwapMinUsd ?? 0.10);
            if (token && token.usd >= autoSwapMinUsd) {
...
async function runSafetyChecks(name, args) {
  switch (name) {
    case "deploy_position": {
      ...
      const maxBinsBelow = Number(config.strategy.maxBinsBelow ?? config.strategy.defaultBinsBelow ?? minBinsBelow);
      if (
        isSingleSidedSol &&
        args.downside_pct == null &&
        Number.isFinite(maxBinsBelow) &&
        requestedBinsBelow > maxBinsBelow
      ) {
        return {
          pass: false,
          reason: `bins_below ${requestedBinsBelow} exceeds configured maxBinsBelow ${maxBinsBelow}. Use configured bins_below only; skip high-volatility pools that need wider range.`,
        };
      }
      if (
        isSingleSidedSol &&
